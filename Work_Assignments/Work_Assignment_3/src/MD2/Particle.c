...
void force(MD *md,Particles *part, int pos){
     ...
     double force[3];      // Guarda as forças
     ...
     for (i = pos + 1; i < md ->mdsize; i++) {
          ...
          if(powDist[0] <= rcoffs) 
          {
               ...
               epotAux += powDist[6] - powDist[3];// (rrd6 - rrd3);
               virAux  += powDist[0] * powDist[8];// rd * r148;
               interAux++;
          }
     }
     ...
     #pragma omp critical
     {
          third_newton_law(part, i, force);
          updateForce(part, force, pos);
     }

     #pragma omp critical
     {
          update_static_variables2(epotAux, virAux, interAux, md);
     }
}
...
void update_static_variables2(double epotAux, double virAux, int interAux, MD *md){
     md->epot += epotAux;// (rrd6 - rrd3);
     md->vir  -= virAux;// rd * r148;
     md->interactions += interAux;
}
